...
best_metric = st.sidebar.selectbox("Pick best model by metric", ["AUC", "F1", "Accuracy", "Precision", "Recall", "MCC"], index=0)
...
ordered_cols = ["ML Model Name", "Accuracy", "AUC", "Precision", "Recall", "F1", "MCC"]
...
best_model_path = save_model_pkl(fitted[best_name], out_path="models/best_model.pkl")
...
if st.sidebar.button("💾 Save selected model as .pkl"):
    sel_name_sanitized = "".join(c for c in choice if c.isalnum() or c in ("_", "-")).strip("_-")
    sel_path = save_model_pkl(fitted[choice], out_path=f"models/{sel_name_sanitized}.pkl")
    st.sidebar.success(f"Saved: {sel_path}")

sel_bytes = _read_bytes("models/{}.pkl".format("".join(c for c in choice if c.isalnum() or c in ("_", "-")).strip("_-")))
if sel_bytes:
    st.sidebar.download_button(
